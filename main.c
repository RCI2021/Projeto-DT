...
enum state_main {
    ...
    wait, join, join_direct, connected, quit

};

int arg_verify(struct my_info *args, int argc); //Argument Verification
    ...
    while (state != quit) {
        switch (state) {
            ...
            case wait:
                ...
                break;
            case join:
                //TODO Call UDP to fill extern net info
                break;
            case connected:
...
int info_alloc(struct my_info *args, struct net_info *info, char *command) {
    ...
    if ((args->IP = (char *) malloc(sizeof(argv[1]))) == NULL) return 1;//TODO Get sizeofs
    if ((args->TCP = (char *) malloc(sizeof(argv[2]))) == NULL) return 1;
    if ((args->regIP = (char *) malloc(sizeof(argv[3]))) == NULL) return 1;
    if ((args->regUDP = (char *) malloc(sizeof(argv[4]))) == NULL) return 1;
 ...
 **********************************************************************************************************************/
int arg_verify(struct my_info *args, int argc) {
